...
class Other_Model_DbTable_DbBanner extends Zend_Db_Table_Abstract
{

    protected $_name = 'mini_banner';
   ...
   public function getAll(){
        $db=$this->getAdapter();
        $sql="SELECT *  FROM `mini_banner` where status = 1"; 
 ...
 public function getAllBanner($search=null){
    ...
    public function getBannerById($id){
        $db=$this->getAdapter();
        $sql="SELECT *  FROM `mini_banner` WHERE id = ".$db->quote($id);
        $sql.=" LIMIT 1 ";
        $row=$db->fetchRow($sql);
        return $row;
    }
